...
def fit(epochs, model, loss_func, optimizer, scheduler, train_dl, valid_dl, config,
        gradient_clipping_norm, early_stopping_patience, device, output_dir, tensorboard_output_path, logger, tag, wandb_logger=None):
    ...
    for epoch in range(epochs):
        ...
        if wandb_logger:
            ...
            wandb_logger.log_loss_parameters(loss_func, epoch + 1)
            
            # Log training and validation loss
            wandb_logger.log_loss_value(train_loss, epoch + 1, "train")
            wandb_logger.log_loss_value(val_loss, epoch + 1, "val")
            
            # Log metrics directly without modifying keys
            wandb_logger.log_metrics(train_metrics, epoch + 1, "train")
            wandb_logger.log_metrics(val_metrics, epoch + 1, "val")
        ...
        if early_stop.stop_training(epoch):
            ...
            break
    logger.info(f"Training completed. Best {config.val_metric}: {early_stop.best_value:.4f} at epoch {early_stop.best_epoch}")
    logger.epoch = None
